...
describe('Peer', () => {
    ...
    it('should record connection data', (done) => {
        const peer = new Peer({ hostname: 'tangle.com', port: 666, TCPPort: 777, UDPPort: 888 });
    ...
    it('should record multiple connection data', (done) => {
        const peer = new Peer({ hostname: 'tangle.com', port: 666, TCPPort: 777, UDPPort: 888 });
...
function recordPeerConnections (peer, durations, datas) {
    ...
    return durations.reduce((promise, duration, index) => (
        promise.then(() => recordPeerConnection(peer, duration, datas[index]))
    ), Promise.resolve());
}
