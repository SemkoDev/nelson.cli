...
describe('PeerListTest', () => {
    ...
    it('should add to the list correctly, same host, when multiPort allowed', (done) => {
        const list = new PeerList({ temporary: true, multiPort: true, silent: true });
        list.load([
            'somehost.com/1234/345/567', '122.232.223.0/14265/11111/22222'
        ]).then(() => {
            list.add({
    ...
    it('should find in list with unknown remote key, but existing address', (done) => {
        ...
        list.load([
            ...
            list.findByRemoteKeyOrAddress('abcdef', 'somehost.com', 2345).then((peers) => {
                expect(peers).toHaveLength(1);
                done();
            })
        });
    ...
    it('should find in list with known remote key, but unknown address', (done) => {
        ...
        list.add({
            ...
            list.findByRemoteKeyOrAddress('abcdef', 'unknownhost.com', 2345).then((peers) => {
                expect(peers).toHaveLength(1);
                expect(peers[0].data.hostname).toEqual(list.all()[0].data.hostname);
                done();
            })
        });
    });
    ...
    it('should not find in list with unknown remote key and address', (done) => {
        ...
        list.add({
            ...
            list.findByRemoteKeyOrAddress('xyz', 'unknownhost.com', 2345).then((peers) => {
    ...
    it('should find in list correctly by using IP', (done) => {
        const list = new PeerList({ temporary: true, multiPort: false, silent: true });
        list.load([
    ...
    it('should return correct peer trusts', (done) => {
        const list = new PeerList({ temporary: true, ageNormalizer: 60, silent: true });
        list.load([
            'somehost.com/1234/345/567', '122.232.223.0/14265/11111/22222'
        ]).then(() => {
            setTimeout(() => {
    ...
    it('should return correct peer weights', (done) => {
        const list = new PeerList({ temporary: true, ageNormalizer: 60, silent: true });
        list.load([
            'somehost.com/1234/345/567', '122.232.223.0/14265/11111/22222'
        ]).then(() => {
                ...
                list.add({
                    ...
                    setTimeout(() => {
                        list.add({
                            ...
                            const weights = list.getWeighted().map(w => w[1]).sort();
                            weights.reverse();
